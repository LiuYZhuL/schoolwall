		...
		<view class="op" :style="{ position: 'relative', top: '-12px', zIndex: '10' }">
			...
			<view class="headerg" style="margin-top:15px ;margin-left:-10px ;">
				<scroll-view class="scroll-view_H" scroll-x="true">
					<block v-for="(tag, index) in tags" :key="index">
						<text class="tag" :class="{ active: activeTag === tag.name }" @tap="setActiveTag(tag.name)">
							{{ tag.name }}
						</text>
					</block>
			...
			<view class="post-container">
				<view class="post-header">
					...
					<button class="follow-button" @click="followUser(1)">关注TA</button>
				</view>
				<view class="post-content" @click="goto1(postId)">
					<view class="post-tag">
	...
	export default {
		...
		methods: {
			followUser(userId) {
				...
				uni.request({
					url: 'http://127.0.0.1:4523/m1/4600643-4250220-default/api/user/follow/' + userId,
					method: 'POST',
					header: {
			...
			goto1(postId) {
				uni.navigateTo({
					url: '/pages/details/uni-post?postId=' + postId,
				})
			},
			goto() {
				uni.navigateTo({
